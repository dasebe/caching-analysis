...
int main (int argc, char* argv[]) {
  ...
  while (infile >> t >> id >> size) {
    for(size_t skip=0; skip<skipcols; skip++) {
      infile >> skiptmp;
    }
      reqCount++;
  ...
  cerr << "total reqs scanned for freq " << reqCount << "\n";
  reqCount= 0;
  infile.close();
  infile.open(path);
  ...
  for(size_t i=0; i<=objCount; i++) {
    ...
    if(freqs[id]>100) {
      ...
      cerr << "excluding (less than 10) " << fit->second << " " << fit->first << "\n";
    }
    fit++;
  }
  ...
  cerr << "total reqs scanned for intervals " << reqCount << " reps " << index << "\n";
  ...
  // calculate
  std::map<double,uint64_t> pearsonCounts;
  auto vic = vecs.begin();
  ...
  do {
    ...
    while(++lag1 != vecs.end()) {
      ...
      const double coeff = pearsoncoeff(vecptr,vecptr2);
      pearsonCounts[round(10.0*coeff)/10.0]++;
      // cout << path << " " << initCount << " "
